...
- **Assume nothing about the target install.** This skill never touches a cluster — it has no idea what the install ships, defaults to, or has configured (secret-store backend, environments, pipeline, builders, plane config, any "default"). State every such item as a prerequisite the migrator **verifies and creates if missing**, never as "already present" or "X by default." Don't name a specific install profile (k3d, quickstart, etc.).
- **Always recommend authoring CT / RT / Trait — never reference shipped.** Corollary of the above: the skill can't know what ships (no install introspection). Bundled samples in `references/sample-types/` are *shape reference*, not a menu. A foreign CR (apiVersion outside standard K8s + `openchoreo.dev/*` + `gateway.networking.k8s.io/*`) implies an external controller.
